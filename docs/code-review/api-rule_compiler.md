...
| D2 | `_sort_groups` is a **no-op**; rules kept in file order. espyak instead re-sorts at match time by **real phoneme code** (`G2P._sort_rules_by_phoneme_code`) keyed `(group_seq, [code], match_str)`, where `code` is `PhonemeSource.codes` — espeak's actual CompilePhoneme code allocation | `string_sorter` 1174 (sorts by phoneme *string* then match) + matcher last-best-wins `>=` tie-break | JUSTIFIED | C sorts by the compiled phoneme-CODE bytes (EncodePhonemes output); the *matcher* tie-break (sort-last equal scorer wins) is what matters. espyak reproduces the codes exactly (`PhonemeSource._compute_codes`: reserved block 1–33 then first-appearance append, inheritance-copied), not a table-insertion-order proxy — the proxy inverts da `?o`(124)/`?V`(109) and mis-picks `?V` on the 82-point `blokade` tie (→ blʔʌkˈaaðə); real codes pick `?o` (→ blʔokˈaaðə). A plain ASCII-mnemonic sort gets `tn` (b→B) wrong. The `group_seq` key reproduces "later `.group` block wins" (bn duplicate `এ`). Full 105-lang headword audit is a strict subset before→after (only `blokade` removed, 0 added). |
