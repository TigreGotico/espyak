...
# A symbol the language does NOT name must not be isolated into a spurious word.
# English has no entry for £, so "£5" stays whatever it was; what matters is that the
# per-language lookup drives the decision rather than a hardcoded set.
def test_symbol_isolation_is_per_language():
    assert G2P("nl")._symbol_has_name("\u00b0")
    assert not G2P("en")._symbol_has_name("\u00b0")
    assert G2P("en")._symbol_has_name("#")
